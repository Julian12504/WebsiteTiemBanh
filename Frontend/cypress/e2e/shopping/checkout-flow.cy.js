...
describe('E2E: Complete Checkout Flow', () => {
  ...
  it('TC_E2E_CHECKOUT_001: Complete purchase journey from browse to order confirmation', () => {
    ...
    // Step 6: Add to cart
    cy.get('.add-to-cart-btn, .add-to-cart-button').first().should('not.be.disabled').click();
  ...
  it('TC_E2E_CHECKOUT_002: Should add multiple products to cart', () => {
    ...
    cy.get('.item').first().find('.add-to-cart-btn, .add-to-cart-button').should('not.be.disabled').click();
    cy.wait(1000);
    
    // Add second product
    cy.get('.item').eq(1).find('.add-to-cart-btn, .add-to-cart-button').should('not.be.disabled').click();
    cy.wait(1000);
  ...
  it('TC_E2E_CHECKOUT_006: Should validate delivery information', () => {
    ...
    cy.visit('/cart');
    cy.contains('button', /Thanh toán/i).should('not.be.disabled').click();
