...
const ItemBarcode = ({ url }) => {
  ...
  const fetchItemDetails = async () => {
    try {
      ...
      console.error("Lỗi khi lấy sản phẩm in barcode:", err);
      setError(err.response?.data?.message || "Lỗi khi tải dữ liệu sản phẩm");
      toast.error(err.response?.data?.message || "Lỗi khi tải dữ liệu sản phẩm");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePrintMultiple = () => {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>In mã vạch - ${item.name}</title>
          <style>
            body {
    ...
    try {
      ...
      console.error("Lỗi khi tạo mã vạch để in:", err);
    }
    ...
    for (let i = 0; i < copies; i++) {
      printWindow.document.write(`
        <div class="barcode-print-container">
          ...
          <div class="barcode-price">${parseFloat(item.selling_price).toLocaleString('vi-VN')} VNĐ</div>
        </div>
      `);
  ...
  if (error || !item) {
    return (
      <div className="item-barcode-error">
        <div>⚠️ {error || "Không thể tải sản phẩm"}</div>
        <button
          className="back-btn"
    ...
    <div className="item-barcode-container">
      ...
      <div className="barcode-content-container">
        <div className="barcode-details">
          ...
          <p><strong>Giá trị barcode:</strong> {item.barcode || item.sku}</p>
          <p><strong>Danh mục:</strong> {item.category}</p>
          <p><strong>Giá bán:</strong> {parseFloat(item.selling_price).toLocaleString('vi-VN')} VNĐ</p>
        </div>
        ...
        <div className="bulk-print-options">
          <h3>In nhiều tem cùng lúc</h3>
          <div className="copies-selector">
            <label htmlFor="copies">Số lượng tem cần in:</label>
            <input
              type="number"
              id="copies"
              min="1"
