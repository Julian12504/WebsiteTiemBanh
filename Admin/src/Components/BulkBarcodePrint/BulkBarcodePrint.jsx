...
const BulkBarcodePrint = ({ url }) => {
  ...
  const handlePrint = async () => {
    if (selectedItems.length === 0) {
      toast.warning("Vui lòng chọn ít nhất một sản phẩm để in mã vạch");
      return;
    }
    ...
    try {
      ...
      if (itemsData.length === 0) {
        toast.error("Không lấy được dữ liệu sản phẩm để in mã vạch");
        return;
      }
        ...
        for (let i = 0; i < item.copies; i++) {
          ...
          printWindow.document.write(`
            <div class="barcode-print-container">
              ...
              <div class="barcode-price">VNĐ ${parseFloat(item.selling_price).toFixed(2)}</div>
            </div>
          `);
      ...
      console.error("Lỗi khi in mã vạch:", err);
      toast.error("Không thể in mã vạch");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="bulk-barcode-container">
      ...
      <div className="bulk-print-controls">
        ...
        <div className="copies-control">
          <label htmlFor="copies">Số lượng tem mỗi sản phẩm:</label>
          <input
            type="number"
            id="copies"
            min="1"
        ...
        >
          [In] {selectedItems.length} {selectedItems.length === 1 ? 'sản phẩm' : 'các sản phẩm'}
        </button>
      </div>
            ...
            {filteredItems.length > 0 ? (
              ...
              <tr>
                <td colSpan="5" className="no-items-message">
                  {searchTerm ? "Không có sản phẩm phù hợp" : "Không có sản phẩm"}
                </td>
              </tr>
            )}
          </tbody>
